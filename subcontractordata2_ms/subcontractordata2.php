	...
	if ($mDB->rowCount() > 0) {
		while ($row=$mDB->fetchRow(2)) {
			...
			if ($mDB2->rowCount() > 0) {
				...
				$subcontractor_id5 = $row2['subcontractor_name'];
			}
			$Qry2="SELECT subcontractor_name FROM subcontractor WHERE subcontractor_id = '$subcontractor_id6'";
			$mDB2->query($Qry2);
			if ($mDB2->rowCount() > 0) {
				$row2=$mDB2->fetchRow(2);
...
$fellow_count = 0;
//取得指定管理人數
$pjmyfellow_row = getkeyvalue2($site_db."_info","pjmyfellow","web_id = '$web_id' and project_id = '$project_id' and auth_id = '$auth_id' and pro_id = 'subcontractordata2'","count(*) as fellow_count");
...
$pjItemManager = false;
//檢查是否為指定管理人
$pjmyfellow_row = getkeyvalue2($site_db."_info","pjmyfellow","web_id = '$web_id' and project_id = '$project_id' and auth_id = '$auth_id' and pro_id = 'subcontractordata2' and member_no = '$memberID'","count(*) as enable_count");
...
<script type="text/javascript" charset="utf-8">
	...
	$(document).ready(function() {
		...
		$('#db_table').dataTable( {
			...
			"ordering": false,
			"ajaxSource": "/smarty/templates/$site_db/$templates/sub_modal/project/func02/subcontractordata2_ms/server_subcontractordata2.php?site_db=$site_db&fm=$fm",
			"language": {
						"sUrl": "$dataTable_de"
			...
			"fnRowCallback": function( nRow, aData, iDisplayIndex ) { 
				...
				$('td:eq()', nRow).html( '<div class="d-flex justify-content-center align-items-center text-center size12 weight text-nowrap" style="height:auto;min-height:32px;">'+case_id+'</div>' );


				//放樣發包進度
				var subcontracting_progress2 = "";
				if (aData[17] != null && aData[17] != "")
					subcontracting_progress2 = aData[17];

				$('td:eq(5)', nRow).html( '<div class="d-flex justify-content-center align-items-center text-center size12" style="height:auto;min-height:32px;">'+subcontracting_progress2+'</div>' );
				...
				//下包放樣
				var subcontractor_id5 = "";
				if (aData[6] != null && aData[6] != "")
					subcontractor_id5 = aData[6];

				$('td:eq(6)', nRow).html( '<div class="size12 text-center" style="width: 100%; word-break: break-all; white-space: normal; min-height:32px;">'+subcontractor_name5+'</div>' );
				...
				$('td:eq(9)', nRow).html( '<div class="size12 text-center" style="width: 100%; word-break: break-all; white-space: normal; min-height:32px;">'+subcontractor_name7+'</div>' );
				...
				$('td:eq(12)', nRow).html( '<div class="size12 text-center" style="width: 100%; word-break: break-all; white-space: normal; min-height:32px;">'+subcontractor_name8+'</div>' );
				...
				$('td:eq(15)', nRow).html( '<div class="size12 text-center" style="width: 100%; word-break: break-all; white-space: normal; min-height:32px;">'+subcontractor_name6+'</div>' );
